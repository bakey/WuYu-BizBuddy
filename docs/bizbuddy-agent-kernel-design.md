...
- 管理 DSH 子进程
- 配置安环 Persona 和 Skills
- 经企业模型入口调用模型（不持有供应商 API Key）
- 注册文档检索与 DCS 查询工具
- 把 DSH 事件转换成 BizBuddy 事件
- 保存会话和工具调用记录
- 向主程序提供本机轨迹读取（`listTraces` / `readTrace`）
...
DSH Runtime
  ├─ Agent Loop
  ├─ 企业模型入口（无供应商 Key；clientToken 由主程序注入）
  ├─ 安环 Persona 与 Skills
  ├─ doc_search
  ├─ ts_list
  ├─ ts_query
  └─ SQLite
        │
        ├─ ragBaseUrl → 企业入口 → 文档检索
        └─ collectorBaseUrl → 企业入口 → 采集
```

DSH 运行在独立 Node 子进程中。`BizBuddyAgentKernel` 负责启动、通信和关闭，桌面端统一调用稳定的外层接口，DSH 内部调整由 B 模块消化。

`KernelLaunchConfig` 不包含供应商 `apiKey`。`model.baseUrl` 为企业模型入口；`model.model` 可空。管理员更改企业默认模型后，新的 `prompt` 不必 `shutdown`。更换 Skills 仍须本轮结束后 `shutdown` / `start`。
...
字段与页面 IPC 见 `docs/agent-kernel-interface-protocol.md` 和 `docs/MVP_Desktop_AE_Module_Detail.md`。本机 `TraceSummary` 不含管理端列表多出的问题、引用数；由主程序计算后交给企业入口。
...
`doc_search` 接收查询内容和返回数量，经 `ragBaseUrl`（企业入口）调用文档检索。结果包含原文片段、文档名称、页码、引用编号和片段编号。B 模块把结构化引用单独传给页面，用于跳转数据库页面并高亮对应文件。

MVP 阶段通过 `documentId` 跳转数据库页面并高亮对应文件。正式运行文档服务不可用时返回 `RAG_UNAVAILABLE`，不用 mock 检索冒充答案。
...
`ts_query` 根据测点编号和时间范围查询历史采样值。时间统一使用带时区的 ISO 8601，回答中的数值以接口结果为准。`collectorBaseUrl` 空则返回 `DCS_UNAVAILABLE`。

## 对外事件

| 事件 | 用途 |
|---|---|
| `searching` | 表示正在查询文档或 DCS 数据；须带 `callId` 和 `tool` |
| `text_delta` | 逐段返回回答文字 |
| `citation` | 返回可定位的文档引用 |
| `done` | 表示本轮处理完成 |
| `error` | 返回模型、工具、用户中止或进程错误 |

无工具的计划不发 `searching`，只进本机轨迹。A/E 与 B 模块协议文档记录具体字段和一次完整调用示例，本设计保留事件职责和架构关系。
...
1. 按对接协议实现会话列表、快照、删除与 `abort` / `onExit`。
2. 启动配置去掉供应商 `apiKey`，改为企业模型入口与 `clientToken`。
3. 替换文档服务、采集的真实接口，完成桌面联调。

## 产品介绍摘要

B 模块以 DSH 为运行基础，负责 BizBuddy 的 Agent 处理过程。文档问答和 DCS 问答已经通过 mock 服务跑通，下一阶段接入真实业务接口并补完会话与中断。
